...
fs.appendFile('../../json/TRAIN_betas_spearman.json', '{\n', function (err) { if (err) console.log(err) })
...
var s1 = fs.createReadStream('../../json/TRAIN_betas.json')
    ...
    .pipe(es.mapSync(function (line) {
        ...
        if (line.toString().trim().startsWith("\"cg")) {
            ...
            spearmanRHO.calc()
                .then(value => {
                    fs.appendFile('../../json/TRAIN_betas_spearman.json', `\t"${line.toString().trim().slice(0, 12).replaceAll("\"", "")}": ${value},\n`, function (err) {
        ...
        .on('end', function () {
            fs.appendFile('../../json/TRAIN_betas_spearman.json', '}', function (err) { if (err) console.log(err) })
            console.log("done")
        })
    )