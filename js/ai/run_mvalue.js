...
for (let n of ns) {
    ...
    let totalError = 0
    const sites = JSON.parse(fs.readFileSync(`../../json/n=${n}/TEST_mvalues_high (${n}).json`, 'utf8'))
    for (let i = 0; i < testSampleAge.length; i++) {    // person
        let res = 0
        let count = n
        let errCount = 0
        for (let site in sites) {
            if(sites[site].length != testSampleAge.length) {
                count--
                errCount++
                continue
            }
            const net = new brain.NeuralNetwork()
            const model = JSON.parse(fs.readFileSync(`./model_mvalue/n=${n}/${site}.json`, 'utf8'))
            net.fromJSON(model)
            res = res + Math.round(net.run([sites[site][i]]) * 100)
        }
        res = res / n
        //console.log(`Result for sample ${i}(${testSampleAge[i]}): ${res} | ${errCount} errors`)
